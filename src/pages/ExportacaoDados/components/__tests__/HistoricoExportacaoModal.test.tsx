...
jest.mock("../../hooks/useExportacaoVagas", () => ({
  useExportacaoVagas: jest.fn(() => ({
    listData: {
      count: 2,
      results: [
        {
        ...
        } as IExportacaoVagasListItem,
        {
          ...
          concurso_nome: null,
          cargo_nome: null,
        } as unknown as IExportacaoVagasListItem,
      ],
    },
    listLoading: false,
...
describe("HistoricoExportacaoModal", () => {
  it("deve renderizar título correto para vagas-processo", () => {
    render(<HistoricoExportacaoModal open onClose={jest.fn()} tipo="vagas-processo" />);

    expect(screen.getByTestId("modal-title")).toHaveTextContent(
      "Histórico de exportações - Vagas Processo",
    );
  });

  it("deve renderizar título correto para vagas-sigpec", () => {
    render(<HistoricoExportacaoModal open onClose={jest.fn()} tipo="vagas-sigpec" />);

    expect(screen.getByTestId("modal-title")).toHaveTextContent(
      "Histórico de exportações - Vagas SIGPEC",
    );
  });
  ...
  it("deve chamar onClose ao fechar o modal", async () => {
    ...
    render(<HistoricoExportacaoModal open onClose={onClose} tipo="vagas-processo" />);
