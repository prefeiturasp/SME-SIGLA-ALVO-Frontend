...
import ExportacaoVagasFormTab from "../ExportacaoVagasFormTab";
...
const renderComponent = (props?: Partial<React.ComponentProps<typeof ExportacaoVagasFormTab>>) => {
  const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });

  return render(
    <QueryClientProvider client={queryClient}>
      <BrowserRouter>
        <ExportacaoVagasFormTab tipo="vagas-processo" {...props} />
      </BrowserRouter>
    </QueryClientProvider>,
  );
};
...
describe("ExportacaoVagasFormTab", () => {
  it("deve renderizar estrutura básica da aba", () => {
    ...
    expect(screen.getByText(/Exportação de vagas/i)).toBeInTheDocument();
  ...
  it("deve renderizar tabela de histórico quando há dados", () => {
    (jest.requireMock("../../hooks/useExportacaoVagas") as any).useExportacaoVagas.mockReturnValue({
      control: {},
      formErrors: {},
      handleSubmit: mockHandleSubmit,
      handleExportar: mockHandleExportar,
      ...
      listData: {
        results: [
          { uuid: "1", nome_arquivo: "arquivo.csv", criado_em: "2024-01-01", status: "SUCESSO" },
        ],
        count: 1,
      },
      listLoading: false,
