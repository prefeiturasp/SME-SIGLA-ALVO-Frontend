...
Then('a resposta ESCOLHA deve ser um objeto', () => {
  cy.get('@response').then((res) => {
...
Then('a resposta ESCOLHA deve conter {string}', (chave) => {
  cy.get('@response').then((res) => {
    const body = res.body
...
Then('o header Content-Type ESCOLHA deve conter {string}', (valor) => {
  cy.get('@response').its('headers').its('content-type').should('include', valor)
})

Then('o tempo de resposta ESCOLHA deve ser menor que {int} milissegundos', (limite) => {
  cy.get('@response').its('duration').should('be.lessThan', limite)
  Cypress.log({ name: 'Performance', message: `Limite: ${limite}ms` })
})

Then('o campo {string} da resposta ESCOLHA deve ser um UUID válido', (campo) => {
  cy.get('@response').then((res) => {
    const body = res.body
...
Then('o campo {string} da resposta ESCOLHA deve ser uma data válida', (campo) => {
  cy.get('@response').then((res) => {
    const body = res.body
    const valor = body[campo]
...
Then('a lista de resultados ESCOLHA não deve estar vazia', () => {
  cy.get('@response').then((res) => {
    const body = res.body
...
Then('a contagem total ESCOLHA deve ser maior que zero', () => {
  cy.get('@response').then((res) => {
    const count = res.body.count
    expect(count, 'count deve ser maior que zero').to.be.greaterThan(0)
    Cypress.log({ name: 'Validação count', message: `Total: ${count}` })
  })
})
